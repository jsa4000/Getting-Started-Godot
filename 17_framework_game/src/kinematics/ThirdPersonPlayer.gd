...
export (bool) var use_camera_view : bool = false
export(NodePath) var camera_path : NodePath setget _set_camera_path
export (float) var max_running_speed = 4
export (float) var max_walking_speed = 1.5
export (float) var max_crouching_speed = 1.2
export (float) var max_pushing_speed = 0.5
export (float) var acceleration = 20
export (float) var angular_acceleration = 5.0
...
onready var raycast_crouch : RayCast = $RayCastCrouch
...
onready var skeleton_torch : SkeletonIK = $Armature/Skeleton/SkeletonHand
onready var torch : Spatial = $Armature/Skeleton/HandBone/Torch
var torch_interpolation_speed : float = 10

onready var skeleton_head : SkeletonIK = $Armature/Skeleton/SkeletonHead
onready var target_head : Spatial = $Armature/Skeleton/HeadBone/Target
onready var skeleton_torso : SkeletonIK = $Armature/Skeleton/SkeletonTorso
onready var target_torso : Spatial = $Armature/Skeleton/TorsoBone/Target
...
onready var raycast_stairs_min : RayCast = $Armature/RayCastStairsMin
onready var raycast_stairs_max : RayCast = $Armature/RayCastStairsMax
var step_smooth : float = 0.25
var step_max_height : float = 1

onready var raycast_wall_right : RayCast = $Armature/RayCastWallRight
onready var raycast_wall_left : RayCast = $Armature/RayCastWallLeft
onready var skeleton_right_hand : SkeletonIK = $Armature/Skeleton/SkeletonRightHand
onready var target_right_hand : Spatial = $Armature/Skeleton/RigthHandPosition
onready var skeleton_left_hand : SkeletonIK = $Armature/Skeleton/SkeletonLeftHand
onready var target_left_hand : Spatial = $Armature/Skeleton/LeftHandPosition
var offset_hand_position : float = 0.05
var lean_speed : float = 10

onready var skeleton_turn : SkeletonIK = $Armature/Skeleton/SkeletonSpine
onready var target_turn: Spatial = $Armature/Skeleton/SpineBone/Target
var turn_amount : float = 15
var turn_friccion_amount : float = 0.05
var previous_direction : Vector3 = Vector3.INF
var turn_interpolation_speed : float = angular_acceleration * 2
var turn_friccion : float = 0

onready var raycast_right_foot_pos : Spatial = $Armature/Skeleton/RightFootBone/RayCastPosition
onready var skeleton_right_foot : SkeletonIK = $Armature/Skeleton/SkeletonRightFoot
onready var target_right_foot : Spatial = $Armature/Skeleton/RigthFootPosition
onready var raycast_left_foot_pos : Spatial = $Armature/Skeleton/LeftFootBone/RayCastPosition
onready var skeleton_left_foot : SkeletonIK = $Armature/Skeleton/SkeletonLeftFoot
onready var target_left_foot : Spatial = $Armature/Skeleton/LeftFootPosition
onready var target_hips_bone : Spatial = $Armature/Skeleton/HipsBone/Target
...
func _physics_process(delta):	
	...
	if previous_direction == Vector3.INF:
		previous_direction = input_direction
	
	var state = _get_state(input_direction)
	
	if not is_jumping && (Input.is_action_pressed("up") || Input.is_action_pressed("left") || Input.is_action_pressed("right") || Input.is_action_pressed("down")):	
	...
	if state != null:
		animator["parameters/locomotion/blend_position"] = animator["parameters/locomotion/blend_position"].linear_interpolate(state, transition_state)
	...
	if input_direction != Vector3.ZERO:
		...
		animator["parameters/locomotion_time/scale"] = lerp(animator["parameters/locomotion_time/scale"], 1, acceleration * turn_friccion * delta)
	else:
		turn_friccion = 0
		velocity = Vector3(0.0, velocity.y, 0.0)
		#velocity = lerp(velocity, Vector3.ZERO, friction * delta) 
		animator["parameters/locomotion_time/scale"] = lerp(animator["parameters/locomotion_time/scale"], 0, 1/friction * delta)
	...
	if Input.is_action_just_pressed("jump") && is_on_floor():
		animator["parameters/jump_transition/current"] = jump_type.running if is_running else jump_type.idle
...
func _set_target_lookat(target : Spatial, pivot : Spatial, skeleton_ik : SkeletonIK, weight : float, delta: float):
	if target != null:		
...
func _step_on_wall(delta):
	...
	var raycast : RayCast = raycast_wall_right
	var ray_length : float = raycast_right_length
	var skeleton_hand : SkeletonIK = skeleton_right_hand
	var other_hand : SkeletonIK = skeleton_left_hand
	var target_hand : Spatial = target_right_hand
...
func _check_foot_enabled(skeleton_ik : SkeletonIK, foot_state : int, is_foot_colliding: bool, is_feet_in_step: bool):
	if (is_feet_in_step && is_foot_colliding) || foot_state == FeetStates.BOTTOM:
		if foot_state != FeetStates.BOTTOM && foot_state != FeetStates.TOP: 
...
func _move_foot(skeleton_ik : SkeletonIK, target : Spatial, target_position : Vector3, target_normal : Vector3, enabled : bool, delta : float) -> bool:
	if not skeleton_ik.is_running():
...
func _reset_feet_position(skeleton_ik : SkeletonIK):
...
func _get_state(input_direction):		
	...
	if is_crouching && raycast_crouch.is_colliding():
		var state
		if Input.is_action_pressed("up") || Input.is_action_pressed("left") || Input.is_action_pressed("right") || Input.is_action_pressed("down"):	
			state = locomotion_state.crouch
			_set_collsion_state(collision_state.crouch)
			is_crouching = true
		else:
			state = locomotion_state.crouch_idle
			_set_collsion_state(collision_state.crouch_idle)
		return state
	...
	if Input.is_action_pressed("up") || Input.is_action_pressed("left") || Input.is_action_pressed("right") || Input.is_action_pressed("down"):	
		if Input.is_action_pressed("left") && Input.is_action_pressed("right") || Input.is_action_pressed("up") && Input.is_action_pressed("down"):
			...
			_set_collsion_state(collision_state.idle)
			is_idle = true
		elif Input.is_action_pressed("run"):
			state = locomotion_state.run
			_set_collsion_state(collision_state.run)
			is_running = true
			...
			state = locomotion_state.push
			_set_collsion_state(collision_state.push)
			is_pushing = true
		else:
			state = locomotion_state.walk
			_set_collsion_state(collision_state.walk)
			is_walking = true
	else:
...
func _convert_direction_from_camera(current_direction: Vector3, camera : Camera) -> Vector3:
	if camera == null:
...
func _create_debug_mesh(key, color : Color):
	...
	var mesh : Mesh = CubeMesh.new()
	mesh.size = Vector3(0.2,0.2,0.2)
	instance.mesh = mesh
	instance.material_override = SpatialMaterial.new()
