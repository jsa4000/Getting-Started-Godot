...
class_name BehaviorTreeStatesComposite, "../../icons/states.svg"
...
func _get_nodes(node : BehaviorTreeNode) -> Array:
	if node is BehaviorTreeStateDecorator && not node.get_transitions().empty():
...
func _change_state(new_node : BehaviorTreeNode) -> void:
	if new_node is BehaviorTreeStateDecorator && new_node != _current_node:
		...
		emit_signal("state_changed",  _current_node.state_name)
