tool
extends Spatial
class_name SkyCloud, "../icons/icon.svg"
...
onready var sky_tex: Sprite = $SkyViewport/Texture
onready var clouds_tex: Sprite = $CloudsViewport/Texture
...
export (bool) var enable_god_rays : bool = false setget _set_god_rays
export (GradientTexture) var sky_gradient_texture: GradientTexture setget set_sky_gradient;
export var sky_rotation: Vector3=Vector3(0.0,0.0,0.0) setget set_sky_rotation
...
export (Color, RGBA) var moon_light :Color = Color(0.6,0.6,0.8,1.0) setget set_moon_light
export (Color, RGBA) var sunset_light :Color = Color(1.0,0.7,0.55,1.0) setget set_sunset_light
export (float, -0.3,0.3,0.000001) var sunset_offset: float=-0.1
export (float, 0.0,0.3,0.000001) var sunset_range: float=0.2
export (Color, RGBA) var day_light :Color = Color(1.0,1.0,1.0,1.0) setget set_day_light
...
func _ready():
	...
	call_deferred("set_sky_rotation",sky_rotation)
	...
	call_deferred("set_enabled", enabled)
...
func set_sky_rotation(value: Vector3):
	sky_rotation = value
	if is_inside_tree():
		world_env.environment.background_sky_rotation = 	sky_rotation * 0.1
	set_time()
...
func set_time():
	if !is_inside_tree() || not enabled || sky_tex == null || clouds_tex == null:
		return
	...
	sun_pos = sun_pos.normalized().rotated(Vector3(0.0,1.0,0.0).normalized(),sky_rotation.y*2.0*PI)
	moon_pos = Vector3(0.0,1.0,0.0).normalized().rotated(Vector3(1.0,0.0,0.0).normalized(),phi) #Same for Moon
	moon_pos = moon_pos.normalized().rotated(Vector3(0.0,1.0,0.0).normalized(),sky_rotation.y*2.0*PI)
	...
	world_env.environment.adjustment_saturation = 1-clouds_coverage*0.5
...
func set_moon_light(value : Color):
	moon_light = value
	set_time()

func set_sunset_light(value : Color):
	sunset_light = value
	set_time()
		
func set_day_light(value : Color):
	day_light = value
	set_time()
...
func set_enabled(value: bool) -> void:
	...
	set_process(enabled)
	if sky_tex != null:
		sky_tex.visible = enabled
	if clouds_tex != null:
		clouds_tex.visible = enabled
	if sun != null:
		sun.visible = enabled
