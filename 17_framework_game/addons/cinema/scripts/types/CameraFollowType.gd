extends CameraType
class_name CameraFollowType

const PREDICTION_LENGTH_THRESHOLD = 0.01
const DeadZonePath := "res://addons/cinema/scripts/DeadZone.tscn"

var COLLISION_RAY_BIT_MASK := BIT(0) | BIT(2)

export(NodePath) var target_path : NodePath
export(NodePath) var lookat_path : NodePath
export (bool) var show_dead_zone : bool = false setget _set_show_dead_zone
export (bool) var follow_enabled : bool = true
export(float, 0.0, 10.0) var smooth_zone_speed : float = 0.8
export(float, 0.0, 5.0) var dead_zone_speed : float = 0.4
export (bool) var lookat_enabled : bool = false
export (bool) var lookat_at_beginning : bool = false
export(float, 0.0, 10.0) var lookat_smooth_zone_speed : float = 0.5
export(float, 0.0, 5.0) var lookat_dead_zone_speed : float = 0.25
export (Vector3) var lookat_offset : Vector3 = Vector3.ZERO
export(Rect2) var smooth_zone : Rect2 = Rect2(Vector2(0.05,0.05),Vector2(0.9,0.9)) setget _set_smooth_zone
export(Rect2) var dead_zone : Rect2 = Rect2(Vector2(0.3,0.25),Vector2(0.4,0.5)) setget _set_dead_zone
export(Vector2) var zone_offset : Vector2 = Vector2.ZERO setget _set_zone_offset
export(float, 0.0, 1.0) var transition_zone_speed : float = 0.1
export (bool) var collision_enabled : bool = true
export(float, 0.0, 50.0) var collision_slope_speed : float = 30.0
export(float, 0.0, 1.0,0.0001) var collision_angle_threshold : float = 0.7
export(float, 0.0,10.0,0.0001) var collision_offset_position : float = 0.5

export (bool) var prediction_enabled : bool = false
export(float, 0.0, 5.0) var prediction_scale : float = 1.25
export(float, 0.0, 2.0) var prediction_scale_speed : float = 0.2
export(float, 0.0, 1.0,0.0001) var prediction_threshold : float = PREDICTION_LENGTH_THRESHOLD
...
func _compute_collisioned_position(position: Vector3, speed : float) -> Vector3:
	...
	if collision && not collision.collider is KinematicBody:
...
func _raycast(from: Vector3, to: Vector3) -> Dictionary:
	var space_state = gimbal.get_world().get_direct_space_state() 
...
func _translate(target: Vector3, speed: float) -> void:
	gimbal.global_transform.origin = gimbal.global_transform.origin.linear_interpolate(target, speed)
...
func _get_target_mesh() -> MeshInstance:
