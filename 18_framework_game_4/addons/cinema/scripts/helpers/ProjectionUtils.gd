...
static func get_aabb_projection(mesh : MeshInstance3D, camera : Camera3D) -> Rect2:
...
static func get_aabb_from_mesh(mesh : MeshInstance3D) -> Array:
	var vertices = []
	for i in range(AABB_ENDPOINTS_COUNT):
		vertices.append((mesh.get_aabb().get_endpoint(i) * mesh.scale) + mesh.global_transform.origin)
	return vertices

static func get_camera_projections(vertices : Array, camera : Camera3D) -> Array:
	var points = []
...
static func array_to_string(name: String, array : Array):
	...
	return "%s: %s" % [name, String(";").join(string)]
